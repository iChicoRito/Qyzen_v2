{{-- Student upload timeline — layout from demo1 public-profile/profiles/crm.html "Recent Activity".
     Rendered inline by the users index and re-fetched by its poller (admin.users.imports.timeline).
     Root carries data-active=1 while any import is queued/processing so the poller knows to keep going. --}}
@php
...
<div data-import-timeline data-active="{{ $active ? '1' : '0' }}">
                    ...
                    @foreach ($imports as $import)
                        @php
                            [$icon, $tone] = match ($import->status) {
                                'completed' => $import->failed_count > 0
                                ...
                                <div class="flex flex-col">
                                    <div class="text-sm {{ $tone ?: 'text-foreground' }}">
                                        <button type="button" class="font-medium text-mono text-start hover:text-primary"
                                            data-modal-url="{{ route('admin.users.imports.show', $import) }}"
                                            data-modal-target="#form_modal"
                                            data-modal-title="{{ $import->original_filename }}">{{ $import->original_filename }}</button>
                                        @switch($import->status)
                                            @case('completed')
                                                imported — {{ $import->created_count }} created, {{ $import->failed_count }} failed.
                                                @break
                                            @case('processing')
                                                is processing — chunk {{ $import->processed_chunks }}/{{ $import->total_chunks }}.
                                                @break
                                            @case('failed')
                                                failed to import.
                                                @break
    ...
    @else
        <div class="kt-card">
            <div class="kt-card-content text-center text-sm text-secondary-foreground py-8">No student uploads yet.</div>
        </div>
    @endif
</div>

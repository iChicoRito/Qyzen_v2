...
<article class="kt-card">
    <div class="flex items-center gap-3 mb-5 p-7.5 pb-0">
        ...
        <div class="flex flex-col"><span class="text-base font-medium text-mono mb-1">{{ $announcement->educator?->name ?? 'Educator' }}</span><time class="text-sm text-secondary-foreground">{{ $announcement->created_at?->diffForHumans() }}</time></div>
    </div>
    <div class="grid gap-3.5 mb-5 px-7.5">
        ...
        <div class="text-sm text-foreground leading-5.5 announcement-body">{!! $announcement->body !!}</div>
        @if ($images)
